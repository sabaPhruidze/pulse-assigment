    ...
    <Card title="📋 Holdings">
      {loading ? (
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-gray-100">
              {(assets || []).map((a) => (
                <tr key={a.assetId} className="hover:bg-gray-50">
                  <td className="py-3 pr-4 font-semibold text-gray-900">{a.assetId}</td>
                  <td className="py-3 pr-4 text-gray-700">{a.quantity}</td>
                  <td className="py-3 pr-4 text-gray-700 hidden sm:table-cell">{formatCurrency(a.avgBuyPrice)}</td>
                  <td className="py-3 pr-4 text-gray-900">{formatCurrency(a.currentPrice)}</td>
                  <td className="py-3 pr-4 text-gray-700 hidden md:table-cell">{formatCurrency(a.value)}</td>
          ...
          {!assets?.length && <div className="py-8 text-center text-gray-500">No holdings.</div>}
        </div>
      )}
    </Card>
  )
